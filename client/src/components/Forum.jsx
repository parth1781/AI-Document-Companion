      ...
      <div className="session-sidebar" style={{ width: '280px', borderRight: '1px solid var(--border-color)', background: 'var(--card-bg)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '1rem', borderBottom: '1px solid var(--border-color)' }}>
          <button className="btn btn-primary" style={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }} onClick={() => {
             setSessionId(null); setTopic(''); setMessages([]);
          }}>
             <ArrowLeft size={16} style={{transform: 'rotate(135deg)'}} /> New Discussion
          </button>
        </div>
      ...
      <main className="main-layout" style={{ flex: 1, flexDirection: 'column', overflow: 'hidden', padding: '1rem 2rem' }}>
        ...
        <div className="card" style={{ padding: '1.5rem', marginBottom: '1rem' }}>
          ...
          <textarea
            style={{
              ...
              resize: 'none', fontFamily: 'inherit', outline: 'none', background: '#f8fafc'
            }}
            placeholder="Paste a blog post, article, or general topic here. The entire discussion revolves around this..."
            value={topic}
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem', overflowX: 'auto', paddingBottom: '0.5rem' }}>
          {PERSONAS.map(p => (
            <button
              key={p.id}
              onClick={() => handlePersonaTabClick(p.id)}
              disabled={isLoading || !topic.trim()}
              style={{
                padding: '0.75rem 1.5rem',
                borderRadius: '8px',
                border: `1px solid ${activePersona === p.id ? 'var(--primary)' : 'var(--border-color)'}`,
                background: activePersona === p.id ? 'var(--primary)' : 'var(--card-bg)',
                color: activePersona === p.id ? 'white' : 'var(--text-main)',
                cursor: (isLoading || !topic.trim()) ? 'not-allowed' : 'pointer',
                fontWeight: 600,
                whiteSpace: 'nowrap',
                transition: 'all 0.2s',
                boxShadow: activePersona === p.id ? '0 4px 6px -1px rgba(79, 70, 229, 0.2)' : 'none'
              }}
            >
              Ask {p.name}
            </button>
        ...
        <div className="card chat-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div className="chat-history" style={{ flex: 1, padding: '1.5rem', overflowY: 'auto' }}>
            {messages.length === 0 && (
              <div style={{ textAlign: 'center', color: 'var(--text-muted)', marginTop: '2rem' }}>
                Enter a topic above, then click a Persona tab to start the discussion!
              </div>
            )}
          ...
          <form className="chat-input-area" onSubmit={handleSubmit}>
            <div className="chat-input-form">
              <input
                ...
                placeholder={topic ? "Add your own thoughts to the discussion..." : "Please enter a topic first..."}
                value={input}
                onChange={(e) => setInput(e.target.value)}
                disabled={isLoading || !topic.trim()}
              />
              <button type="submit" className="btn btn-icon" disabled={!input.trim() || isLoading || !topic.trim()}>
                <Send size={18} />
              </button>
            </div>
          </form>
